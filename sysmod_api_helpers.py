...
def get_sysmod_projects(server_url, project_id, commit_id):
    ...
    project_ids = mbse4u_sysmlv2_helpers.get_user_defined_concepts(server_url, project_id, commit_id, 'project', 'OccurrenceDefinition', 'SYSMOD::Project')    
    simplified_projects = []
    ...
    for item in project_ids:
        ...
        if isinstance(item, dict):
            ...
            el = mbse4u_sysmlv2_helpers.get_element_fromAPI(mbse4u_sysmlv2_helpers.get_commit_url(server_url, project_id, commit_id), item)
...
def get_system_idea(server_url, project_id, commit_id, element_id):
    ...
    if context_part:
        ...
        print(f"Found context part: {context_part.get('declaredName')}")    
        sub_parts = mbse4u_sysmlv2_helpers.get_contained_elements(server_url, project_id, commit_id, context_part['@id'], 'PartUsage')
        found_elements = mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, sub_parts, 'SYSMOD::Project::systemIdeaContext::systemOfInterest')
...
def get_context_part(server_url, project_id, commit_id, sysmod_project_id, context_type):
    """
    Returns the context part element
    """
    query_url = mbse4u_sysmlv2_helpers.get_commit_url(server_url, project_id, commit_id)
...
def get_full_context(server_url, project_id, commit_id, sysmod_project_id, context_type):
    """
...
def get_context_system(server_url, project_id, commit_id, context_part):
    ...
    context_parts = mbse4u_sysmlv2_helpers.get_owned_usages(server_url, project_id, commit_id, context_part, 'PartUsage', 'nestedPart')
    print(f"Found {len(context_parts)} parts in context {context_part.get('name')} ({context_part.get('@id')})")
    for cp in context_parts:
        print(f"\tPart: {cp.get('name')} ({cp.get('@id')})")
    ...
    print(f"\n\n-----------------------------\nFound system: {system.get('name')} ({system.get('@id')})")
    system['parts'] = mbse4u_sysmlv2_helpers.get_owned_usages(server_url, project_id, commit_id, system, 'PartUsage', 'ownedPart')    
    return system
...
def get_stakeholders(server_url, project_id, commit_id, sysmod_project_id):
    ...
    sysmod_project = mbse4u_sysmlv2_helpers.get_element_fromAPI(query_url, sysmod_project_id)
    sysmod_project_partusages = mbse4u_sysmlv2_helpers.get_owned_usages(server_url, project_id, commit_id, sysmod_project, 'PartUsage')
    ...
    stakeholders = []
    for p in stakeholder_part_usages:
        print(f"Found Stakeholder: {p.get('name')} {p.get('@id')}")
                
        # Extract attributes
        stakeholder = {
            'name': p.get('name'),
            'description': mbse4u_sysmlv2_helpers.get_element_documentation(server_url, project_id, commit_id, p.get('@id'), True), # Generic Helper
    ...
    print(f"Found stakeholders: {stakeholders}")
    return stakeholders
...
def get_sysmod_status(server_url, project_id, commit_id, sysmod_project_id):
    ...
    ft_ids = mbse4u_sysmlv2_helpers.get_metadata_ids_by_name(server_url, project_id, commit_id, ['featureTree'])
    if ft_ids.get('featureTree'):
        usages = mbse4u_sysmlv2_helpers.get_metadatausage_annotatedElement_ids(server_url, project_id, commit_id, {'featureTree': ft_ids['featureTree']})
        if usages.get('featureTree'):
            status["FEATURE"] = True

    # Get all PartUsages in the sysmod project to optimize context checks
    part_usages = mbse4u_sysmlv2_helpers.get_contained_elements(server_url, project_id, commit_id, sysmod_project_id, 'PartUsage')
    
    # 2. BFAC (Brownfield Context)
    if mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, part_usages, 'brownfieldSystemContext'):
        status["BFAC"] = True
    ...
    # 5. SIC (System Idea)
    if mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, part_usages, 'systemIdeaContext'):
        status["SIC"] = True

    # 6. SC (System Context)
    if mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, part_usages, 'systemContext'):
        status["SC"] = True
    ...
    if mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, part_usages, 'functionalArchitectureContext'):
        status["FUC"] = True

    # 10. LAC (Logical Architecture Context)
    if mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, part_usages, 'logicalArchitectureContext'):
        status["LAC"] = True

    # 11. PAC (Physical Architecture Context / Product Context)
    if mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, part_usages, 'physicalArchitectureContext'):
         status["PAC"] = True
    elif mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, part_usages, 'productContext'):
         status["PAC"] = True
...
def get_sysmod_requirements(server_url, project_id, commit_id, sysmod_project_id):
    ...
    if sysmod_project:
        ...
        req_usages = mbse4u_sysmlv2_helpers.get_elements_fromAPI(query_url, req_usages_ids)
    else:
        req_usages = mbse4u_sysmlv2_helpers.get_elements_byKind_fromAPI(server_url, project_id, commit_id, 'RequirementUsage')
    print(f"Found {len(req_usages)} RequirementUsages")

    # 2. Find one specializing 'problemStatement'
    requirement_specifications = mbse4u_sysmlv2_helpers.find_elements_specializing(server_url, project_id, commit_id, req_usages, 'SYSMOD::Project::systemRequirementSpecification')
    ...
    if requirement_specifications:
        ...
        owned_requirements = mbse4u_sysmlv2_helpers.get_owned_usages(server_url, project_id, commit_id, requirement_specifications[0], 'RequirementUsage')
        print(f"Found {len(owned_requirements)} owned requirements")

    sysmod_requirements = []
    if owned_requirements:
        for req in owned_requirements:
            req_doc = mbse4u_sysmlv2_helpers.get_element_documentation(server_url, project_id, commit_id, req['@id']) or ""
            if req.get('declaredName') != None:
                sysmod_requirements.append({
                "identifier": req.get('shortName'),
                "name": req.get('declaredName'),
                "uri": req.get('@id'),
                "text": req_doc,
                "motivation": mbse4u_sysmlv2_helpers.get_feature_value(server_url, project_id, commit_id, req, 'motivation') or "",
                "priority": mbse4u_sysmlv2_helpers.get_feature_value(server_url, project_id, commit_id, req, 'priority') or "",
                "obligation": mbse4u_sysmlv2_helpers.get_feature_value(server_url, project_id, commit_id, req, 'obligation') or "",
                "stability": mbse4u_sysmlv2_helpers.get_feature_value(server_url, project_id, commit_id, req, 'stability') or ""
            })
    
    return sysmod_requirements  

def get_sysmod_needs(server_url, project_id, commit_id, sysmod_project_id):
    """
    Retrieves needs from the model.
    """

    need_usages_ids = mbse4u_sysmlv2_helpers.get_user_defined_concepts(server_url, project_id, commit_id, 'extendedConcern', 'ConcernDefinition', 'SYSMOD::ExtendedConcern')    
    print(f"\n\nfound need usages: {len(need_usages_ids)}\n\n")
    
    needs = []
    for need_usage_id in need_usages_ids:
        need = mbse4u_sysmlv2_helpers.get_element_fromAPI(mbse4u_sysmlv2_helpers.get_commit_url(server_url, project_id, commit_id), need_usage_id)
        need_doc = mbse4u_sysmlv2_helpers.get_element_documentation(server_url, project_id, commit_id, need['@id'], True)

        need_stakeholders = ""
        need_parameters = mbse4u_sysmlv2_helpers.get_owned_parameters(server_url, project_id, commit_id, need, 'stakeholderParameter')
        for need_parameter in need_parameters:
            if need_stakeholders == "":
                need_stakeholders += need_parameter.get('declaredName')
            else:
                need_stakeholders += ", " + need_parameter.get('declaredName')

        needs.append({
            "identifier": need.get('shortName'),
            "name": need.get('declaredName'),
            "text": need_doc,
            "stakeholders": need_stakeholders,
            "priority": mbse4u_sysmlv2_helpers.get_feature_value(server_url, project_id, commit_id, need, 'priority') or "",
        })
...
def get_sysmod_usecases(server_url, project_id, commit_id, sysmod_project_id    ):
    ...
    """
    requirement_system_context = get_context_part(server_url, project_id, commit_id, sysmod_project_id, 'SYSMOD::Project::requirementsContext')
    use_cases = mbse4u_sysmlv2_helpers.get_owned_usages(server_url, project_id, commit_id, requirement_system_context, 'UseCaseUsage')
    if use_cases:
        use_cases = [uc for uc in use_cases if uc.get('name') != 'useCases']
    ...
    for use_case in use_cases:
        ...
        objective = mbse4u_sysmlv2_helpers.get_owned_usages(server_url, project_id, commit_id, use_case, "RequirementUsage", "objectiveRequirement", includeInherited = True)
        use_case_doc = mbse4u_sysmlv2_helpers.get_element_documentation(server_url, project_id, commit_id, objective[0]['@id'], True)

        actors = mbse4u_sysmlv2_helpers.get_owned_parameters(server_url, project_id, commit_id, use_case, "actorParameter")
        actor_names = ""
        for a in actors:
            if actor_names == "":
                actor_names += a.get('name')
            else:
                actor_names += ", " + a.get('name')

        steps = mbse4u_sysmlv2_helpers.get_owned_usages(server_url, project_id, commit_id, use_case, "ActionUsage")
        step_names = ""
        for s in steps:
            print(f"Found step: {s}")
            if step_names == "":
                if s.get('name') != None:
                    step_names += s.get('name')
            else:
                if s.get('name') != None:
                    step_names += ", " + s.get('name')


        use_cases_json.append({
            "name": use_case.get('name'),
            "description": use_case_doc,
            "actors": actor_names,
            "steps": step_names
        })  

    print(f"\n\nFound {len(use_cases_json)} use cases\n\n {use_cases_json}")
    return use_cases_json


def call_ai(prompt, api_key, org_id=None):
    """
    Calls OpenAI API to get a suggestion based on the prompt.
    """
    try:
...
def generate_image(prompt, api_key, org_id=None):
    """
    Calls OpenAI API to generate an image based on the prompt.
    """
    try:
        from openai import OpenAI
    except ImportError:
...
def clean_ai_response(text):
    ...
    # Normalize newlines to avoid double spacing issues
    text = text.replace('\r\n', '\n').replace('\r', '\n')
